...
public class NativeTestCase : XCTestCase {
    ...
    func featureScenarioTest() {
        guard let (feature, scenario) = self.dynamicType.featureScenarioData(self.invocation!.selector) else {
            return
        }
        ...
        if let defined = feature.background?.stepDescriptions.map(self.state.matchingGherkinStepExpressionFound).reduce(true, combine: { $0 && $1 }) {
